...
class ModelWrapper(nn.Module):
    ...
    def forward(
        ...
        for chunk in chunks(
            ...
            greedy_native_scores = list(
                getattr(decoder, "last_generate_scores", [None] * bs)
            )
            ...
            generation_metadata = [
                [
                    {
                        "generation_source": "greedy",
                        "native_model_score": greedy_native_scores[i]
                        if rank < len(greedy_native_scores)
                        else None,
                        "candidate_rank": rank,
                    }
                    for rank, _ in enumerate(generations[i])
                ]
                for i in range(bs)
            ]
            ...
            if self.beam_type == "search":
                ...
                for i in range(bs):
                    decoded = []
                    decoded_metadata = []
                    for rank, (native_score, hyp) in enumerate(search_generations[i]):
                        tree = env.idx_to_infix(
                            hyp.cpu().tolist()[1:],
                            is_float=False,
                            str_array=False,
                        )
                        ...
                        decoded_metadata.append(
                            {
                                ...
                                "native_model_score": float(native_score),
                                "candidate_rank": rank,
                            }
                        )
                    generations[i].extend(decoded)
                    generation_metadata[i].extend(decoded_metadata)
                ...
                sampling_native_scores = list(
                    getattr(decoder, "last_generate_scores", [None] * (bs * num_samples))
                )
                ...
                for i in range(bs):
                    ...
                    generation_metadata[i].extend(
                        {
                            "generation_source": "sampling",
                            "native_model_score": sampling_native_scores[
                                i * num_samples + rank
                            ]
                            if i * num_samples + rank < len(sampling_native_scores)
                            else None,
                            "candidate_rank": rank,
                        }
                        for rank, _ in enumerate(sampling_generations[i])
                    )
            else:
                raise NotImplementedError
            outputs.extend(generations)
            metadata_outputs.extend(generation_metadata)
        self.last_generation_metadata = metadata_outputs
        return outputs
