...
try:
    ...
    # 兼容旧环境中 functorch 未单独安装的情况。
    # 这里的用法仅依赖单参数标量目标函数梯度，autograd 已足够。
    def grad(func):
        def _wrapped(coeffs):
            if not isinstance(coeffs, torch.Tensor):
                coeffs = torch.tensor(coeffs, dtype=torch.float64, requires_grad=True)
            elif not coeffs.requires_grad:
                coeffs = coeffs.clone().detach().requires_grad_(True)

            objective = func(coeffs)
            if objective is None:
                raise ValueError("BFGS objective returned None")

            gradient, = torch.autograd.grad(objective, coeffs, create_graph=False, retain_graph=False)
            return gradient
