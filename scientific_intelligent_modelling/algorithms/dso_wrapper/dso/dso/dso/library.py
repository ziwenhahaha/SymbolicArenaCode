...
class Polynomial(Token):
    ...
    def eval_poly(self, X):
        ...
        # sklearn/linear-regression paths may leave coefficients as a column
        # vector. DSO binary operators expect 1-D program outputs; returning
        # (N, 1) here would broadcast with (N,) into an accidental (N, N).
        return np.asarray(np.dot(self.eval_monomials(X, self.exponents), self.coef)).reshape(-1)
