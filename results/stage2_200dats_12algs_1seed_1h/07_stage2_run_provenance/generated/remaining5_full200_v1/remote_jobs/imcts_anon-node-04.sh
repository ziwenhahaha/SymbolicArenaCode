...
conda run -n sim_iMCTS python check/launch_e1_benchmark.py run \
  --tool iMCTS \
  --slice-csv "$REMOTE_ROOT/exp-planning/02.E1选择验证/generated/remaining5_full200_v1/slices/imcts/anon-node-04.csv" \
  --params-json "$REMOTE_ROOT/exp-planning/02.E1选择验证/generated/params/imcts.json" \
  --output-root "$REMOTE_ROOT/experiments/${BATCH_NAME}/imcts/anon-node-04" \
  --seed 1314 \
  --workers "$WORKERS" \
  "${EXTRA_ARGS[@]}"
