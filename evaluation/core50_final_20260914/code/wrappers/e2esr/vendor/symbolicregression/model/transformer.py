...
class TransformerModel(nn.Module):
    ...
    def generate(
        ...
        sequence_log_probs = src_enc.new_zeros(bs).float()
        ...
        while cur_len < max_len:
            ...
            token_log_probs = F.log_softmax(scores.float(), dim=1).gather(
                1, next_words.unsqueeze(1)
            ).squeeze(1)
            sequence_log_probs.add_(token_log_probs * unfinished_sents.float())
        ...
        token_counts = (gen_len.float() - 1.0).clamp_min(1.0)
        self.last_generate_scores = (
            sequence_log_probs / token_counts
        ).detach().cpu().tolist()
